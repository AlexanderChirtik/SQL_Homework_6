...
DROP PROCEDURE IF EXISTS friendship_proposals;
DELIMITER //
CREATE PROCEDURE friendship_proposals(for_user_id BIGINT)
BEGIN
WITH friends AS (
...
CALL friendship_proposals(2);
-- Взял код из презинтации, но изучил структуру и даже переписал интересные моменты кода
...
SELECT friendship_direction(7);
-- Также взял код из презинтации, внимательно его изучив
...
DROP FUNCTION IF EXISTS times_of_day;
DELIMITER //
CREATE FUNCTION times_of_day()
RETURNS VARCHAR(15) READS SQL DATA 
BEGIN
DECLARE now_hour INT DEFAULT EXTRACT(hour FROM now());
DECLARE result VARCHAR(15);

	IF now_hour BETWEEN 0 AND 5
		THEN SET result = "Доброй ночи";
	ELSEIF now_hour BETWEEN 6 AND 11
		THEN SET result = "Доброе утро";
	ELSEIF now_hour BETWEEN 12 AND 17
		THEN SET result = "Добрый день";
	ELSEIF now_hour BETWEEN 18 AND 23
